...
The project-local queue reserves expected canonical WU-C2 PR #4, branch onecompany-a4-wu-c2, exactly docs/onecompany-fixture/WU-C22.md. This is only a reservation; GitHub's ACTUAL PR number must be independently checked before any lease event or worker operation. If it differs, STOP, amend project-local base policy, and rebind to the actual PR — never relabel a foreign PR. No fixture PR, lease, CI failure, review or merge exists at installation time.

Leave ONECOMPANY_A4_PRODUCER_ENABLED unset/false (WU-C2 is already PR-bound) and ONECOMPANY_L2_FIXTURE_REPAIR_ENABLED unset/false until actual initial claim PR, intentional failed exact-head CI log, native owner-published ROLE_LEASE_ASSIGNED admission, independent reviewer capacity and dispatcher readiness are separately established. Do not forge operational events or treat the reserved PR number as proof.

After verified native admission, owner opts in ONECOMPANY_L2_FIXTURE_REPAIR_ENABLED=true, ONECOMPANY_A4_APPROVED_DISPATCHER=NTinkicht, ONECOMPANY_A4_LOGICAL_ACTOR=fixture-bot. Repair workflow appends only Repair: complete to the exact WU-C2 fixture and fails closed on missing authority. Re-run CI on repaired SHA and verify independent non-author approval, controlled merge and ledger MERGED event before claiming L2.

Preflight note: setup PR #3 was closed unmerged without dispatch after upstream source CodeRabbit findings; this replacement reservation targets actual future PR #4. Never count #3 as pilot evidence.

Canonical replacement setup uses WU-C2 and onecompany-a4-wu-c2. The abandoned WU-C branch remains untouched for audit; never delete it or force-update it. Target PR #4 must be the sole WU-C2 experiment.
